...
final class AuditLog
{
    ...
    public function write(string $event, array $context = [], ?array $user = null): void
    {
        ...
        Files::withLock($this->file . '.lock', function() use ($event,$context,$user): void {
            $previousHash = '';
            $headFile = $this->file . '.head';
            if (is_file($headFile)) {
            ...
            file_put_contents($this->file, json_encode($entry, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE) . PHP_EOL, FILE_APPEND);
            file_put_contents($headFile,$entry['hash'].PHP_EOL,LOCK_EX);
