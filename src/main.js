...
function app(){
  return {
    ...
    verificationErrorHint(job){
      ...
      if(/OVERLAY_BACKUP_FAILED|OVERLAY_RESTORE/.test(code))return 'DigiOps could not safely back up or restore an existing private file; publication was stopped to avoid a mixed-version tree.'
      if(/COPY_FAILED/.test(code))return 'A filesystem copy failed at the exact relative path shown. The appended PHP filesystem reason identifies the failing source/destination condition.'
    ...
    <main class="min-w-0 flex-1">
      ...
      <div class="px-4 py-6 md:px-7">
        ...
        <section x-show="page==='project' && selected">
          ...
          <div x-show="projectTab==='settings'" class="panel"><h2 class="font-bold">Application settings</h2><div class="mt-5 grid gap-4 md:grid-cols-2"><div><span class="muted">Repository</span><b class="mt-1 block" x-text="selectedRepo"></b></div><div><span class="muted">Branch</span><b class="mt-1 block" x-text="selectedBranch"></b></div><div><span class="muted">Artifact</span><b class="mt-1 block" x-text="selectedArtifactName"></b></div><div><span class="muted">Health path</span><b class="mt-1 block" x-text="selectedHealthPath"></b><small class="mt-1 block break-all text-slate-500">Resolved: <code x-text="selectedHealthUrl"></code></small></div><div><span class="muted">Deployment target</span><b class="mt-1 block" x-text="targetName(selectedTargetId)"></b></div><div><span class="muted">Public path</span><code class="mt-1 block text-xs" x-text="selectedPublicPath"></code></div><div><span class="muted">Private path</span><code class="mt-1 block text-xs" x-text="selectedPrivatePath"></code></div></div></div>
        </section>
