...
function fail(string $error,int $status=400): never { http_response_code($status); echo json_encode(['ok'=>false,'error'=>$error],JSON_UNESCAPED_SLASHES); exit; }
...
function publishRelease(array $payload,string $zipFile): array {
    ...
    try{
        ...
        $tmp=dirname($publicTarget).'/.'.$slug.'.publish-'.bin2hex(random_bytes(4));copyDir($releaseDir.'/public',$tmp);if($privatePayload!==null){ensureDir($privateTarget);copyDir($releaseDir.'/private',$privateTarget);}if(is_dir($publicTarget))removeTree($publicTarget);ensureDir(dirname($publicTarget));if(!rename($tmp,$publicTarget))fail('PUBLISH_RENAME_FAILED',500);
        writeJsonFile($runtime.'/current.json',['release'=>$releaseId,'commit'=>(string)($payload['commit']??''),'lastDeploy'=>date(DATE_ATOM)]);removeTree($stage);return['release'=>$releaseId];
...
try{
    if($action==='ping')ok(['agentVersion'=>AGENT_VERSION,'capabilities'=>['health','releases','files','deploy-chunked','rollback'],'runtime'=>['php'=>PHP_VERSION,'curl'=>extension_loaded('curl'),'zip'=>extension_loaded('zip')],'server'=>php_uname('n')]);
